...
int para_equal(para* p, para* q) {
  ...
  //Fixed for when the same file is passed (Would SEGFAULT before)
  while (p->base[i] != '\0' && q->base[j] != '\0' && ((equal = strcmp(p->base[i], q->base[j])) == 0)){ 
    ++i;
    ++j;
    }
...
void para_print(para* p, void (*fp)(const char*, int flags[]), int flags_arr[]) {
  if (p == NULL) { 
    return; 
  }

  for (int i = p->start; i <= p->stop && i != p->filesize; ++i) { 
      //printf("Printhing is in here \n");
      fp(p->base[i], flags_arr); 
    }
}

void para_printfile(char* base[], int count, void (*fp)(const char*, int flags_arr[]), int flags_arr[]) {
  para* p = para_first(base, count);
  while (p != NULL) {
    para_print(p, fp, flags_arr);
    p = para_next(p);
  }
  printline();
}
